...
class pid_control:
    def __init__(self, P=0.003, I=0.003, D=0.00, derivator=0, integrator=0, integrator_max=3000, integrator_min=-3000):
        self.Kp = P
        self.Ki = I
        self.Kd = D
...
ax_data_bin.grid()
ax_data_bin.set_ylim(-5000, 65635 + 5000)
ax_data_bin.set_ylabel('Data')
line_data_bin, = ax_data_bin.plot(xp_data, 'g')    # green line
ax_data_value = ax_data_bin.twinx()                # show another Y-axis (voltage value)
ax_data_value.set_ylim(-10.24 - 1.5625, 10.24 + 1.5625)
...
for i in range(len(xp_data)):
    ...
    out = int(out)
    out = out + 0x8000
    out = 65535 if out > 65535 else out
